...
frame_grab(void *data)
{
	...
	if (pos == 0)
		{
			...
			/* calculate the three section points */
			p1 = (len * 1.0) / 4.0;
			p2 = (len * 2.0) / 4.0;
			p3 = (len * 3.0) / 4.0;
			if (len < 120.0)
	...
	/* don't need the extra crap on my screen...
	...
	*/
	...
	if (ef)
		{
			/* write the frame data to the eet file if the file is open. */
	...
	if (pos == 1)
		{
			if ((p - p1) >= snip)
				{
					/* if our movie short is now beyond our snip start + snip len,
	...
	if ((frnum >= 1) && (oneframe))
		{
			/* if we were instructed to just grab one frame, then
			 * exit after that frame was grabbed.
			 */
			ecore_main_loop_quit();
		}
	return EINA_TRUE;
